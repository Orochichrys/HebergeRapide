    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
           ...
           <p className="text-gray-400 mt-1">Gérez vos sites et surveillez les performances.</p>
        </div>
        <Link 
          to="/deploy" 
      ...
      {deployments.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-dark-border rounded-xl bg-dark-card/30">
          ...
          <h3 className="text-xl font-medium text-white mb-2">Aucun déploiement actif</h3>
          <p className="text-gray-400 mb-6">Vous n'avez pas encore hébergé de site. Lancez-vous maintenant !</p>
            ...
            <div key={deploy.id} className="bg-dark-card border border-dark-border rounded-xl p-6 hover:border-brand-500/50 transition-all group">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`px-2 py-1 rounded text-xs font-medium ${
                  deploy.status === 'live' ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400'
                }`}>
                  {deploy.status === 'live' ? 'EN LIGNE' : 'EN COURS'}
                </div>
              </div>
              
              <h3 className="text-lg font-bold text-white mb-1 truncate">{deploy.name}</h3>
              <a href={deploy.url} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-400 hover:text-brand-400 flex items-center gap-1 mb-4 truncate">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                 <div>
                    ...
                    <span className="text-white font-mono">{deploy.visitors}</span>
                 </div>
                 <div>
                    <span className="text-xs text-gray-500 block mb-1 flex items-center gap-1">
